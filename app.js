...
async function saveData(type) {
    ...
    showToast("✅ දත්ත සාර්ථකව සුරැකිණි!");
...
function viewCodeDetails(code, type) {
    ...
    <div style="margin-bottom: 20px;">
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 20px;">
            <div style="background: ${type === 'IN' ? '#d4edda' : '#f8d7da'}; padding: 15px; border-radius: 8px; text-align: center;">
                <div style="font-size: 14px; color: ${type === 'IN' ? '#155724' : '#721c24'};">මුළු ${type === 'IN' ? 'ලැබීම්' : 'ගෙවීම්'}</div>
                <div style="font-size: 24px; font-weight: bold; color: ${type === 'IN' ? 'green' : 'red'};"> ${total.toLocaleString(undefined, {minimumFractionDigits: 2})}</div>
            </div>
            <div style="background: #e2e3e5; padding: 15px; border-radius: 8px; text-align: center;">
                <div style="font-size: 14px; color: #383d41;">ගනුදෙනු ගණන</div>
                <div style="font-size: 24px; font-weight: bold; color: #383d41;">${transactions.length}</div>
            </div>
        </div>
        
        <h4 style="color: var(--primary); border-bottom: 2px solid #eee; padding-bottom: 5px;">ගනුදෙනු ලැයිස්තුව</h4>
    `;
    
    if (transactions.length === 0) {
        html += `<p style="text-align: center; color: #666; padding: 20px;">ගනුදෙනු කිසිවක් නැත</p>`;
    } else {
        html += `
        <table style="width: 100%; border-collapse: collapse; margin-top: 10px;">
            <thead>
                <tr style="background: ${type === 'IN' ? '#d4edda' : '#f8d7da'};">
                    ...
                    <th style="padding: 10px; border: 1px solid #ddd; text-align: left;">${type === 'IN' ? 'ලදුපත් අංකය' : 'වවුචර් අංකය'}</th>
        ...
        transactions.sort((a, b) => new Date(b.date) - new Date(a.date)).forEach(tr => {
            ...
            <tr style="border-bottom: 1px solid #eee;">
                ...
                <td style="padding: 8px; border: 1px solid #ddd;">${type === 'IN' ? tr.ref : (tr.vouch || tr.ref)}</td>
                <td style="padding: 8px; border: 1px solid #ddd;">${tr.proj || '-'}</td>
                <td style="padding: 8px; border: 1px solid #ddd; text-align: right; font-weight: bold; color: ${type === 'IN' ? 'green' : 'red'};">${tr.amt.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
            </tr>`;
        });
        ...
        html += `
            ...
            <tfoot>
                <tr style="background: ${type === 'IN' ? '#c3e6cb' : '#f5c6cb'}; font-weight: bold;">
                    <td colspan="4" style="padding: 10px; border: 1px solid #ddd; text-align: right;">මුළු එකතුව:</td>
                    <td style="padding: 10px; border: 1px solid #ddd; text-align: right; color: ${type === 'IN' ? 'green' : 'red'};"> ${total.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                </tr>
            </tfoot>
        </table>`;
    }
...
async function refreshDashboard() {
    ...
    document.getElementById('dash-in').innerText =  tin > 0 ? tin.toLocaleString(undefined, {minimumFractionDigits:2}) : ' - ';
    document.getElementById('dash-ex').innerText = tex > 0 ? tex.toLocaleString(undefined, {minimumFractionDigits:2}) : ' - ';
    document.getElementById('dash-bal').innerText =  (tin-tex) > 0 ? (tin-tex).toLocaleString(undefined, {minimumFractionDigits:2}) : ' - ';
    let fundHtml = '';
    S_CODES.forEach((s, i) => {
        const bal = db.filter(r => r.source === s).reduce((a,b) => a + (b.type==='IN'?b.amt:-b.amt), 0);
        fundHtml += `<div class="fund-box" style="background:${COLORS[i%COLORS.length]}"><div>${s}</div><div style="font-size:18px;"> ${bal > 0 ? bal.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) : ' - '}</div><small>${CODE_INFO[s]}</small></div>`;
    });
...
function resetForms() {
    ...
    document.getElementById('btn-save-in').innerText = "ලැබීම ගබඩා කරන්න";
    document.getElementById('btn-save-ex').innerText = "ගෙවීම ගබඩා කරන්න";
}
